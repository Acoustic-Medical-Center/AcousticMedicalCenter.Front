...
export class PatientSettingsFormComponent {
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes['patientId'] && this.patientId !== null) {
      this.patientFormSettings.patchValue({ id: this.patientId });
  ...
  loadPatientSettings(): void {
    this.service.getPatientById(this.patientId).subscribe(
      (settings: IPatient) => {
        ...
        console.log('patientFormSettings Value', this.patientFormSettings.value);
      },

      (error: any) => {
        this.toastr.error('Error fetching patient settings');
        console.log(error);
      },
    );
  }
