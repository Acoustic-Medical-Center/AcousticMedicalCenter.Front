...
export class PatientGeneralSettingsFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.userId) {
      this.loadPatientGeneralSettings(this.userId);
    }else{
      console.log('User Id bulunamadı');
    }
  } 

  loadPatientGeneralSettings(userId: string) {
  ...
  onSubmit(): void {
    if (this.settingsGeneralPatientForm.valid && this.userId) {
